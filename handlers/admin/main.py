...
from database.methods.user import parseAllUsers, make_new_admin
...
async def __send_AdPost(query: CallbackQuery, state: FSMContext):
    ...
    await bot.send_message(admin_id, 'Рассылка начата✅', reply_markup=await kb_main(admin_id))
    complete_message = await broadcaster(bot, msgText, image, video)
    await bot.send_message(admin_id, complete_message, reply_markup=await kb_main(admin_id))
